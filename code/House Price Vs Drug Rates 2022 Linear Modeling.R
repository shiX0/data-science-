...
ggplot(house_price_drug_crime_data,aes(x=`Drug Offence Rate`,y=Price)) +
  ...
  geom_point(data = filter(house_price_drug_crime_data,County=="KENT"),aes(color=c("Green"="Kent")))+ #setting color as red for Kent's data point
  ...
  labs(x="Drug Offence Rate",
       y="Price",
       title="2022 House Prices vs Drug Offence Rate",color="County") +theme_minimal()
