...
ggplot(house_price_broadband_data,aes(x=`Average download speed (Mbit/s)`,y=Price)) +
  ...
  geom_point(data = filter(house_price_broadband_data,County.x=="KENT"),aes(color=c("Green"="Kent")))+ #setting color as red for Kent's data point
  geom_point(data = filter(house_price_broadband_data,County.x=="SURREY"), aes(color=c("Yellow"="Surrey"))) + 
  ...
  labs(x="Average Download Speed (Mbit/s)",
       y="Price",
       title="2020 House Prices vs Average Download Speed",color="County")+theme_minimal()
